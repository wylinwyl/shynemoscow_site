...
error_log('Request received: ' . print_r($_GET, true) . ' - ' . file_get_contents('php://input'));
header('Content-Type: application/json');

try {
    $service = new service(
        'PcZ45c1xW2vW6c0Hr0wgoT7QUQdBSYaX',    // Ваш реальный логин
        'W3sCX5GYIVb3Nb2JK7uTYNYoT02axjdn'     // Ваш реальный пароль
    );
...
class service {
    ...
    public function __construct($login, $secret, $baseUrl = 'https://api.cdek.ru/v2') { // Боевой сервер
        $this->login = $login;
        $this->secret = $secret;
    ...
    private function getAuthToken() {
        ...
        // Отладка: выводим ответ от API СДЭК
        error_log('CDEK Auth Response: ' . $token['result']);

        $result = json_decode($token['result'], true);

        if (!isset($result['access_token'])) {
            throw new RuntimeException('Server not authorized to CDEK API: ' . ($result['error_description'] ?? 'No token received'));
        }
    ...
    private function httpRequest($method, $data, $useFormData = false, $useJson = false) {
        ...
        if ($useFormData) {
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($data)); // Используем http_build_query для form-data
        } elseif ($useJson) {
